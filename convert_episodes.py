...
def process_files():
    base_dir = r"c:\Users\wonders\Desktop\webNovele\publicAgent_S1"
    src_dir = os.path.join(base_dir, "src")
    
    # Task 1: Ep01 (Special case location)
    ep1_path = os.path.join(base_dir, "episode1", "Ep01_추락_Complete.md")
    if os.path.exists(ep1_path):
        print(f"Processing Episode 1: {ep1_path}")
        with open(ep1_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        html_content = parse_markdown_to_html(content, 1)
        
        out_path = os.path.join(base_dir, "episode1", "index.html")
        with open(out_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"Written to {out_path}")

    # Task 2: Src files
    files = glob.glob(os.path.join(src_dir, "Ep*_Complete.md"))
    for file_path in files:
        filename = os.path.basename(file_path)
        # Extract number "Ep02_..." -> 2
